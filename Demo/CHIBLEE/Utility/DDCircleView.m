...
- (void)animateSwipe
{
    if (!_animationShouldStop) {
        ...
        if (self.swipeDirection == kCircleSwipeRightToLeft) {
            scale = CGAffineTransformMakeScale(2, 2);
            self.transform = scale;
        }
        else if (self.swipeDirection==kCircleSwipeDownToUp)
        {
            scale = CGAffineTransformMakeScale(4,4);
            translateRight = CGAffineTransformMakeTranslation(0,260);
            ...
            // Start on the right hand side as well as scaling
              scale = CGAffineTransformMakeScale(2, 2);
             translateRight = CGAffineTransformMakeTranslation(260, 0);

            self.transform = CGAffineTransformConcat(translateRight, scale);
        }
        self.alpha = 0.0f;
        [UIView animateKeyframesWithDuration:0.7 delay:0.3 options:0
                                  animations:^{
                                      // Fade In
